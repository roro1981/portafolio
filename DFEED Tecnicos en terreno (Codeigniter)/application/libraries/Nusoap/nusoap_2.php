				...
				case 'service':
					switch ($name) {
					    case 'port':
					        ...
					        break;
					    case 'address':
					        //$this->ports[$this->currentPort]['location'] = str_replace("http", "https", $attrs['location']);
					        $this->ports[$this->currentPort]['location'] = $attrs['location'];
					        $this->ports[$this->currentPort]['bindingType'] = $namespace;
					        $this->bindings[ $this->ports[$this->currentPort]['binding'] ]['bindingType'] = $namespace;
					        //$this->bindings[ $this->ports[$this->currentPort]['binding'] ]['endpoint'] = str_replace("http", "https", $attrs['location']);
		...
		switch ($name) {
			case 'import':
			    if (isset($attrs['location'])) {
                    //$this->import[$attrs['namespace']][] = array('location' => str_replace("http", "https", $attrs['location']), 'loaded' => false);
                    $this->import[$attrs['namespace']][] = array('location' => $attrs['location'], 'loaded' => false);
                    //$this->debug('parsing import ' . $attrs['namespace']. ' - ' . str_replace("http", "https", $attrs['location']) . ' (' . count($this->import[$attrs['namespace']]).')');
	...
	function serialize($debug = 0)
	{
		...
		if (count($this->ports) >= 1) {
			foreach($this->ports as $pName => $attrs) {
				...
				//$xml .= "\n" . '    <soap:address location="' . str_replace("http", "https", $attrs['location']) . ($debug ? '?debug=1' : '') . '"/>';
				$xml .= "\n" . '    <soap:address location="' . $attrs['location'] . ($debug ? '?debug=1' : '') . '"/>';
				//$sUrl = str_replace("http", "https", $attrs['location']);
				//$xml .= "\n" . '    <soap:address location="' . $sUrl . ($debug ? '?debug=1' : '') . '"/>';
				$xml .= "\n" . '  </port>';
			} 
		} 
		$xml .= "\n" . '</service>';
